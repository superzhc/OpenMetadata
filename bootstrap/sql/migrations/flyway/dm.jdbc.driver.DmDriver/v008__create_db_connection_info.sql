...
SET json = REPLACE(json,
                   '"name":"' || JSON_UNQUOTE(JSON_EXTRACT(json, '$.name')) || '"',
                   '"name":"' || REPLACE(JSON_UNQUOTE(JSON_EXTRACT(json, '$.name')), ':', '') || '"')
WHERE JSON_UNQUOTE(JSON_EXTRACT(json, '$.serviceType')) = 'Dagster';

UPDATE pipeline_entity
SET json = REPLACE(json,
                   '"fullyQualifiedName":"' || JSON_UNQUOTE(JSON_EXTRACT(json, '$.fullyQualifiedName')) || '"',
                   '"fullyQualifiedName":"' || REPLACE(JSON_UNQUOTE(JSON_EXTRACT(json, '$.fullyQualifiedName')), ':', '') || '"')
WHERE JSON_UNQUOTE(JSON_EXTRACT(json, '$.serviceType')) = 'Dagster';
