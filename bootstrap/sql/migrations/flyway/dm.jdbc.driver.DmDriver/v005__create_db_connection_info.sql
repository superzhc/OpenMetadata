...
CREATE TABLE IF NOT EXISTS user_tokens (
     token VARCHAR(36) GENERATED ALWAYS AS (json_value(json , '$.token')) NOT NULL,
     userId VARCHAR(36) GENERATED ALWAYS AS (json_value(json , '$.userId')) NOT NULL,
     tokenType VARCHAR(50) GENERATED ALWAYS AS (json_value(json , '$.tokenType')) NOT NULL,
     json text NOT NULL CHECK (json IS JSON(LAX)),
     expiryDate BIGINT   GENERATED ALWAYS AS (json_value(json , '$.expiryDate'))NOT NULL
);
...
ADD COLUMN supported_data_types GENERATED ALWAYS AS (json_value(json , '$.supportedDataTypes'));
