package org.openmetadata.service.migration.dameng.v111;
...
public class Migration extends MigrationProcessImpl {
  ...
  public void runDataMigration() {
    String getSql =
        "SELECT t1.id AS id1, t2.id AS id2 FROM test_suite t1 JOIN test_suite t2 ON JSON_UNQUOTE(JSON_EXTRACT(t1.json, '$.executableEntityReference.fullyQualifiedName')) = JSON_UNQUOTE(JSON_EXTRACT(t2.json, '$.executableEntityReference.fullyQualifiedName')) AND t1.id != t2.id";
    ...
    String resultListSql =
        "select json from test_suite  where JSON_UNQUOTE(JSON_EXTRACT(json, '$.executable')) = 'true'";
