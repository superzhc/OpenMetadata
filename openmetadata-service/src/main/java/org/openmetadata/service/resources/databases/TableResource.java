...
public class TableResource extends EntityResource<Table, TableRepository> {
  ...
  @GET
  @Path("/treeByDWGlossary")
  public Response treeByDWGlossary(
      @Context UriInfo uriInfo,
      @Context SecurityContext securityContext,
      @Parameter(
              description = "Filter tables by database fully qualified name",
      ...
      @Parameter(
          ...
          @QueryParam("databaseSchema")
          String databaseSchemaParam,
      @Parameter(description = "数仓分层所属分类", schema = @Schema(type = "string", example = "DataWarehouse"))
          @QueryParam("dwGlossary")
    ...
    return repository.treeByGlossary(dwGlossary, filter);
  }
